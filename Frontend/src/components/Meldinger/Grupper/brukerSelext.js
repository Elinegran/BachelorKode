...
// Funksjon for å opprette en nytt gruppemedlem i databasen
function SelextBruker2() {
    const [idbruker, setIDbruker] = useState(0);

    // Sender det nye idbrukeren til backend
    const addGruppe = () => {
      axios.post("http://localhost:3001/api/meldingerMineGrupper", { idbruker: idbruker, })   
    };
  
    // Dette sendes til Meldingssiden
    return (

        <select className="custom-select" id = "valgtGruppe" onChange = {(event) => {setGruppeID(event.target.value);}}> {/* Her mangler det en onChange event, tror jeg */}
