...
// Funksjon 
function SelextGruppe2() {
    const [gruppeID, setGruppeID] = useState(0);
    const [gruppeliste, setGruppeliste] = useState([]);

    // Henter gruppeID og gruppenavn fra backend
    const getGruppe = () => {
    ...
    return (
        ...
        <select className="custom-select" onChange = {(event) => {setGruppeID(event.target.value);}}> {/* Her mangler det en onChange event, tror jeg */}
