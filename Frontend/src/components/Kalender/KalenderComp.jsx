...
export default class KalenderComp extends React.Component {
  ...
  handleSluttChange(event) {
    this.setState({slutt: event.target.value });
  }
  ...
  const veileder = this.state.veileder;
    ...
    return (
      <div className='demo-app-sidebar'>
        ...
        <input type="datetime-local" id="slutt" name="sutt" onChange={this.handleSluttChange}/>
  ...
  handleUpdate = (info) => {
    if (!alert("Vil du flytte " + info.event.title + " til kl: "+ info.event.start.toISOString() +"?")) {
  ...
  handleSave = (event) => {
    ...
    alert("You are submitting " + this.state.title + "beskrivele " + this.state.beskrivelse + this.state.sted + this.state.start);
  ...
  handleDateSelect =   (selectInfo) => {
    ...
    this.setState({
      ...
      slutt: selectInfo.endStr
    })

    alert("Dette er tiden :" + this.state.start + 'til' + this.state.slutt)
    // if (title) {
